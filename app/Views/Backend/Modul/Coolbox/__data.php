<table id="example" class="table table-hover table-bordered">
    <thead>
        <?php
        $arrth = ['#', 'Kode coolbox', 'Instansi', 'Status', 'Tgl & Jam', 'Keterangan', 'Foto', 'Is_active', 'Actions'];
        echo '<tr>';
        foreach ($arrth as $th) :
            echo '<th>' . $th . '</th>';
        endforeach;
...
<script>
     function editData(id) {
        $.ajax({
            type: 'get',
            url: '<?= site_url('coolbox/posisi-coolbox/edit-data/'); ?>' + id,
            dataType: 'json',
    ...
    function addFoto(id) {
        $.ajax({
            type: 'get',
            url: '<?= site_url('coolbox/posisi-coolbox/add-foto/'); ?>' + id,
            dataType: 'json',
            success: function(response) {
                if (response.sukses) {
                    $(".view-modal").html(response.sukses).show();
    ...
    function deleteData(id) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: 'delete',
                    url: '<?= site_url('coolbox/posisi-coolbox/delete-data/'); ?>' + id,
                    dataType: 'json',
                    success: function(response) {
                        if (response.sukses) {
                            Swal.fire({
